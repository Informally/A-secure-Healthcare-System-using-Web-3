...
async function validateRole(expectedRole) {
    ...
    try {
        if (expectedRole === "doctor") {
            ...
            const nurse = await contractInstance.methods.get_nurse(publicKey).call();
            if (!nurse[0]) throw new Error("Access denied. Nurse role required.");
    ...
    if (logoutButton) {
        logoutButton.addEventListener("click", async () => {
            ...
            try {
                ...
                } else if ((await contractInstance.methods.get_nurse(publicKey).call())[0]) {
                    await contractInstance.methods.log_nurse_logout().send({ from: publicKey });
                    console.log("Nurse logged out successfully.");
                } else {
